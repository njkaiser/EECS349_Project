...
from config import WORKSPACE, LOG_DIR, MODEL_SAVE_DIR, IMAGE_SIZE, NUM_CLASSES, NUM_CHANNELS, CONV1_NUM_FILTERS, CONV1_KERNEL_SIZE, CONV1_ACTIV_FUNC, CONV1_STRIDE, CONV1_PADDING, POOL1_FILTER_SIZE, POOL1_STRIDE, POOL1_PADDING, CONV2_NUM_FILTERS, CONV2_KERNEL_SIZE, CONV2_ACTIV_FUNC, CONV2_STRIDE, CONV2_PADDING, POOL2_FILTER_SIZE, POOL2_STRIDE, POOL2_PADDING, FC1_ACTIV_FUNC, FC1_NUM_NEURONS, LEARNING_RATE, NUM_ITERS, BATCH_SIZE, DROPOUT_RATE, KEEP_PROB, CONV3_NUM_FILTERS, CONV3_KERNEL_SIZE, CONV3_ACTIV_FUNC, CONV3_STRIDE, CONV3_PADDING, POOL3_FILTER_SIZE, POOL3_STRIDE, POOL3_PADDING, CONV4_NUM_FILTERS, CONV4_KERNEL_SIZE, CONV4_ACTIV_FUNC, CONV4_STRIDE, CONV4_PADDING, POOL4_FILTER_SIZE, POOL4_STRIDE, POOL4_PADDING
...
def architecture_4(model_name, configuration, output_file):
    with tf.Session() as sess:
        model_name = model_name
        model_dir = MODEL_SAVE_DIR + model_name + '/'
        log_dir = LOG_DIR + model_name + '/'
        ...
        # convolve the result of h_pool1 with the weight tensor, add the bias, apply the ReLU function, and finally max pool
        h_conv3 = CONV3_ACTIV_FUNC(conv2d(h_pool2, W_conv3, CONV3_STRIDE, CONV3_PADDING) + b_conv3)
        h_pool3 = max_pool_2x2(h_conv3, POOL3_FILTER_SIZE, POOL3_STRIDE, POOL3_PADDING)


        # third convolutional layer
        W_conv4 = weight_variable([CONV4_KERNEL_SIZE[0],CONV4_KERNEL_SIZE[1],CONV3_NUM_FILTERS,CONV4_NUM_FILTERS], 'W_conv4')
        b_conv4 = bias_variable([CONV4_NUM_FILTERS], 'b_conv4')

        tf.summary.image('W_conv4', tf.transpose(W_conv4[:, :, 0:1, :], [3, 0, 1, 2]), max_outputs=CONV4_NUM_FILTERS)

        # convolve the result of h_pool1 with the weight tensor, add the bias, apply the ReLU function, and finally max pool
        h_conv4 = CONV4_ACTIV_FUNC(conv2d(h_pool3, W_conv4, CONV4_STRIDE, CONV4_PADDING) + b_conv4)
        h_pool4 = max_pool_2x2(h_conv4, POOL4_FILTER_SIZE, POOL4_STRIDE, POOL4_PADDING)

        p4s = h_pool4.get_shape().as_list()

        # densely connected layer
        W_fc1 = weight_variable([p4s[1]*p4s[2]*CONV4_NUM_FILTERS, FC1_NUM_NEURONS], 'W_fc1')
        b_fc1 = bias_variable([FC1_NUM_NEURONS], 'b_fc1')

        h_pool4_flat = tf.reshape(h_pool4, [-1, p4s[1]*p4s[2]*CONV4_NUM_FILTERS])
        h_fc1 = FC1_ACTIV_FUNC(tf.matmul(h_pool4_flat, W_fc1) + b_fc1)
...
def architecture_5(model_name, configuration, output_file):
    with tf.Session() as sess:
        model_name = model_name
        model_dir = MODEL_SAVE_DIR + model_name + '/'
        log_dir = LOG_DIR + model_name + '/'
...
def architecture_6(model_name, configuration, output_file):
    with tf.Session() as sess:
        model_name = model_name
        model_dir = MODEL_SAVE_DIR + model_name + '/'
        log_dir = LOG_DIR + model_name + '/'
        ...
        # convolve the result of h_pool1 with the weight tensor, add the bias, apply the ReLU function, and finally max pool
        h_conv3 = CONV2_ACTIV_FUNC(conv2d(h_conv2, W_conv3, CONV3_STRIDE, CONV3_PADDING) + b_conv3)
        h_pool3 = max_pool_2x2(h_conv3, POOL3_FILTER_SIZE, POOL3_STRIDE, POOL3_PADDING)
